;;;; Emacs Bookmark Format Version 1 ;;;;
;;; This format is meant to be slightly human-readable;
;;; nevertheless, you probably don't want to edit it.
;;; -*- End Of Bookmark File Format Version Stamp -*-
(("org-capture-last-stored"
 (filename . "~/org/perlesSoline.org")
 (front-context-string . "**** Nouvelle vo")
 (rear-context-string . "16-04-22 Friday\n")
 (position . 1787))
("rootPi"
 (filename .
           #("/sudo:root@192.168.1.89:/root/" 6 10
             (tramp-default t)))
 (front-context-string . ".Xauthority\n  -r")
 (rear-context-string . " 0 Sep 13 18:00 ")
 (position . 183))
("home.org"
 (filename . "~/org/home.org")
 (front-context-string . "** Dump some mem")
 (rear-context-string . "4\n* Information\n")
 (position . 32840))
("piInternet"
 (filename . "/ssh:pi@89.84.80.182#22002:/home/pi/")
 (front-context-string . "WebSite\n  drwxr-")
 (rear-context-string . "0K Feb 23  2015 ")
 (position . 1942))
("butor"
 (filename . "/ssh:ng860516@ufrsciencestech.u-bourgogne.fr|ssh:ng860516@butor:/home1/ng860516/")
 (front-context-string . ".gconfd\n  drwxr-")
 (rear-context-string . "0K Sep 23 09:52 ")
 (position . 1212))
("ufr"
 (filename . "/ssh:ng860516@ufrsciencestech.u-bourgogne.fr:/home1/ng860516/")
 (front-context-string . ".matlab\n  drwx--")
 (rear-context-string . "0K Sep 24  2014 ")
 (position . 2065))
("dhclient.conf"
 (filename . "/sudo:root@localhost:/etc/dhcp/dhclient.conf")
 (front-context-string . "prepend domain-n")
 (rear-context-string . "ervers 8.8.8.8;\n")
 (position . 853))
("pi"
 (filename . "/ssh:pi@192.168.1.89:/home/pi/")
 (front-context-string . ".Xauthority\n  -r")
 (rear-context-string . "57 Sep 23 18:38 ")
 (position . 183))
("public_html"
 (filename . "~/public_html/")
 (front-context-string . "boids.js\n  drwxr")
 (rear-context-string . " juil. 16 02:42 ")
 (position . 222))
(".emacs.d/settings"
 (filename . "~/.emacs.d/settings/")
 (front-context-string . "appearance.el\n  ")
 (rear-context-string . " sept.  8 02:02 ")
 (position . 226))
)